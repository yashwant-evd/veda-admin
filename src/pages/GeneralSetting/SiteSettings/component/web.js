...
import { WebValidate, _initialValues } from "./utils";
...
function Web() {
  ...
  const { webLoader, web, updateweb } = useSelector((state) => state.web);

  useEffect(() => {
    dispatch(getAllWebAsync({}));
  }, []);
  ...
  const onSubmit = async (values) => {
    ...
    const payload = {
      ...
      enrollmentWord: values.enrollmentWord,
  ...
  useEffect(() => {
    if (web) {
      formik.setFieldValue("favicon", [web.favicon]);
      formik.setFieldValue("siteLogo", [web.siteLogo]);
      formik.setFieldValue("siteMiniLogo", [web.siteMiniLogo]);
      formik.setFieldValue("sitePreloader", [web.sitePreloader]);
      formik.setFieldValue("siteTitle", web.siteTitle);
      formik.setFieldValue("siteAuthorName", web.siteAuthorName);
      formik.setFieldValue("loginImage", [web.loginImage]);
      formik.setFieldValue("siteDescription", web.siteDescription);
      formik.setFieldValue("socialContent", web.socialContent);
      formik.setFieldValue("enrollmentWord", web.enrollmentWord);
      formik.setFieldValue("copyrightText", web.copyrightText);
      formik.setFieldValue("siteKeyword", web.siteKeyword);
    }
  }, [web]);

  useEffect(() => {
    if (updateweb.status === 200) {
      toast.success(updateweb.message, toastoptions);
      dispatch(emptyadmin());
          ...
          <Grid container spacing={4}>
            <Grid item xs={12} md={12}>
              <Card sx={{ p: 3, direction: "start" }}>
                ...
                >
                  <Box>
                    <Typography sx={{ mb: "10px" }}>Enrollment Word</Typography>
                    <FormControl fullWidth>
                      <TextField
                        // size="small"
                        name="enrollmentWord"
                        placeholder="Enrollment Word"
                        {...formik.getFieldProps("enrollmentWord")}
                        onChange={formik.handleChange}
                        fullWidth
                        error={
                          formik.touched.enrollmentWord &&
                          formik.errors.enrollmentWord
                        }
                      />
                    </FormControl>
                    <Typography sx={{ fontSize: "12px", color: "grey" }}>
                      Word should be 3-5 letters and in capital(ex - ACAD).
                    </Typography>
                  </Box>
                  <Box>
                    <Typography sx={{ mb: "10px" }}>Copyright Text</Typography>
                    <FormControl fullWidth>
                      <TextField
                        // size="small"
                        name="copyrightText"
